...
def main():
    ...
    for i in range(0, group_num):
        ...
        progress_str = '%s' % ''.join(map(lambda x: '#', progress[:(progress_len * (i + 1)) // group_num]))
        print(''.join(['[', progress_str, ''.join('-' * (progress_len - len(progress_str))), ']']))
        print('新发现你被%d人删除' % DeletedCount)
        for i in range(DeletedCount):
            if d[DeletedList[i]][1] != '':
                print(d[DeletedList[i]][0].decode('utf-8') + '({0})' .format(d[DeletedList[i]][1].decode('utf-8')))
            else:
    ...
    for r in result:
        if d[r][1] != '':
            resultNames.append(d[r][0].decode('utf-8') + '(%s)' % d[r][1].decode('utf-8'))
        else:
